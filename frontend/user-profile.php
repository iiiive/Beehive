...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!preg_match("/^[a-zA-Z ]+$/", $firstname)) {
        ...
        if (!empty($password)) {
            ...
            $sql = "UPDATE users 
                    SET firstname=?, lastname=?, username=?, email=?, birthday=?, address=?, contact_number=?, password_hash=? 
                    WHERE user_id=?";
            $stmt = $link->prepare($sql);
            $stmt->bind_param("ssssssssi", $firstname, $lastname, $username, $email, $birthday, $address, $contact_number, $password_hash, $user_id);
        } else {
            $sql = "UPDATE users 
                    SET firstname=?, lastname=?, username=?, email=?, birthday=?, address=?, contact_number=? 
                    WHERE user_id=?";
            $stmt = $link->prepare($sql);
...
.success { color: #4c8b27ff; margin-bottom: 30px; }
.error { color: #b42e2eff; margin-bottom: 30px; }
.back-btn {
  position: absolute;
  top: 20px;
  left: 20px;
...
<form method="POST" id="profileForm">
  <div class="form-group">
    <input type="text" name="firstname" id="firstname" placeholder="First Name" 
           value="<?= htmlspecialchars($user['firstname']) ?>" required pattern="[A-Za-z ]+">
  </div>
  <div class="form-group">
    <input type="text" name="lastname" id="lastname" placeholder="Last Name" 
           value="<?= htmlspecialchars($user['lastname']) ?>" required pattern="[A-Za-z ]+">
  </div>
  <div class="form-group">
